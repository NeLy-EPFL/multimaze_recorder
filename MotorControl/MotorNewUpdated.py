...
arduino = serial.Serial(port='/dev/ttyACM1',  baudrate=74880, timeout=.1)
...
def sendTarget(real_target, final_target, posMagnet,arrived, path, zaber_dragging,wrong_input, stopped,pos_remove):
    ...
    try:
        ...
        return real_target, final_target_old, arrived, path, zaber_dragging, stopped, False
    ...
    return real_target, final_target,arrived,path, zaber_dragging, stopped, rest
    ...
    if(k%5000==0):
        ...
        if(not dragging and not zaber_dragging):
            try:    
                ...
                print("Wrong image")
            i=i+1
            real_target, final_target,arrived,path,zaber_dragging, stopped, rest =  sendTarget(real_target, final_target, posMagnet, arrived, path, zaber_dragging, wrong_input,stopped,pos_remove)
            time.sleep(0.1)
