...
datafolder = Path(os.environ.get("MMRECORDER_LOCAL_PATH", Path.home() / "Videos"))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Crop corridors from recorded images")
    parser.add_argument(
        "--data-folder", "-d",
        default=str(datafolder),
        help="Folder containing *_Recorded experiment directories",
    )
    args = parser.parse_args()

    try:
        check_process(Path(args.data_folder))

        if os.isatty(sys.stdin.fileno()):
            run_checkcrops = input("Launch verification of processed folders integrity? (y/n): ")
            if run_checkcrops.lower() == "y":
                subprocess.run([sys.executable, "-m", "multimaze_recorder.processing.check_process",
