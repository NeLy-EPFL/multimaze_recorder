...
def verify_experiments_from_yaml(yaml_file, verification_script):
    ...
    for i, exp_dir in enumerate(directories, 1):
        ...
        try:
            cmd = (
                [sys.executable, str(verification_script), "--folder", str(exp_path)]
                if verification_script
                else [sys.executable, "-m", "multimaze_recorder.processing.verify_processed",
                      "--folder", str(exp_path)]
            )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a verification script (default: uses mmrecorder-verify-processed module)",
    )
